        ...
        if (rule.url.match(/^https?:\/\//)) {
            ...
            var options = {
                ...
                onError: function (err, req, res/* , url*/) {
                    ...
                    if (that.request[url] !== undefined) {
                        that.request[url]--;
                        if (that.request[url] <= 0) delete that.request[url];
                    }
                },
            ...
            that.app.use('/' + that.config.route + rule.regex, function (req, res, next) {
                ...
                if (rule.lastError && ((Date.now() - rule.lastError) < that.config.errorTimeout)) {
                    res.status(404).send('[proxy] Cannot read file: ' + rule.lastErrorText);
                    return
                }

                if (that.request[url] > 10) {
                    res.status(500).send('[proxy] too many parallel requests!');
                    adapter.log.warn('[proxy] too many parallel requests for "' + url + '"');
                    return;
                }
                that.request[url] = that.request[url] || 0;
                that.request[url]++;
