...
function Proxy(server, webSettings, adapter, instanceSettings, app) {
    ...
    this.config.parallelRequests = parseInt(this.config.parallelRequests, 10) || 10;
    ...
    function oneRule(rule) {
        ...
        rule.parallelRequests = parseInt(rule.parallelRequests, 10) || that.config.parallelRequests;
        ...
        if (rule.url.match(/^https?:\/\//)) {
            if (that.simpleProxy) {
                ...
                const options = {
                    ...
                    onProxyReq: (proxyReq /* , req, res */) => adapter.log.debug(`${proxyReq.method}: ${rule.url}${proxyReq.path}`),
                    onProxyRes: (proxyRes, req, res) => {
                        finishReq(rule, req, res);
                        rule.lastError = 0;
                        rule.lastErrorText = '';
                        adapter.log.debug(`[proxy] Response for ${req.url}: ${res.statusCode}(${res.statusMessage})`);
                    },
                    onOpen: (proxySocket) => {
                        proxySocket && proxySocket.on('error', err => {
                            adapter.log.info(`[proxy] Socket Error: ${err}`);
                        });
                    },
                    onProxyReqWs: (proxyReq, req, socket /*, options, head*/) => {
                        adapter.log.debug(`${proxyReq.method} (WS): ${rule.url}${proxyReq.path}`)
                        socket && socket.on('error', err => {
                            adapter.log.info(`[proxy] Socket Error for ${req.url}: ${err}`);
                        });
                    },
                    /*
                    onClose: function () {
                ...
                that.app.use(`/${that.config.route}${rule.regex}`, (req, res, next) => {
                    ...
                    if (rule.request && rule.request.length > rule.parallelRequests) {
                ...
                /*if (server) { // This produces errors, so NOT do!
                    server.on('upgrade', rule.handler.upgrade); // <-- subscribe to http 'upgrade'
                }*/
            }
        } else {
            ...
            that.app.use(`/${that.config.route}${rule.regex}`, (req, res, _next) => {
                ...
                let fileName = rule.url + requestUrl;
                ...
                adapter.log.debug(`[proxy] File lookup for ${that.config.route}${rule.regex}: rule.req=${rule.url}, requestUrl=${requestUrl}, fileName=${fileName}`);
