...
function Proxy(server, webSettings, adapter, instanceSettings, app) {
    ...
    function oneRule(rule) {
        adapter.log.info('Install extension on /' + that.namespace + '/' + rule.regex);
        var options = {
            target:         rule.url,
            ws:             true,
            changeOrigin:   true,
        ...
        options.pathRewrite['^/' + that.namespace + '/' + rule.regex] = '/';

        rule.handler = proxy(options);
        that.app.use('/' + that.namespace + '/' + rule.regex, function (req, res, next) {
